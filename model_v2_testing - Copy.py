...
with st.sidebar:
    ...
    for group_name, indicators in indicator_groups.items():
        slider_value = st.sidebar.slider(f"{group_name}", min_value=0, max_value=100, value=50, step=10)
...
sum_economic = sum(economic_weights.values())
sum_financial = sum(financial_weights.values())
sum_social = sum(social_weights.values())
sum_infrastructure = sum(infrastructure_weights.values())
sum_environmental = sum(environmental_weights.values())
